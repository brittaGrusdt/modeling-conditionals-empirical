...
var USE_DRIFT_KERNEL = data["use_drift_kernel"][0]
...
var gaussianKernelGamma = function(prevVal) {
...
var priorSample = function(par){
  var gamma = USE_DRIFT_KERNEL ?
    (par.includes("gamma") ? {gamma: sample(Uniform({a: 0, b: 1}), {driftKernel: gaussianKernelGamma})} : {}) :
    (par.includes("gamma") ? {gamma: sample(Uniform({a: 0, b: 1}))} : {});

  // log normal distribution for alpha
  var alpha = USE_DRIFT_KERNEL ?
    (par.includes("alpha") ? {alpha: Math.exp(sample(Gaussian({mu: 1.5, sigma: 1}), {driftKernel: gaussianKernel}))} : {}) :
    (par.includes("alpha") ? {alpha: Math.exp(gaussian({mu: 1.5, sigma: 1}))} : {});

  var theta = USE_DRIFT_KERNEL ?
    (par.includes("theta") ? {theta: sample(Beta({a: 4, b: 2}), {driftKernel: betaKernel})} : {}) :
    (par.includes("theta") ? {theta: beta({a: 4, b: 2})} : {});
...
var speaker_utt_type = function(state, displayU){
  ...
  let distr = Infer({method:'enumerate', model: function(){
    ...
    var LL = marginal_LL(best_assertable_u_same_type.utt, globalStore.thresholds)
...
var run_rsa_model = function(data){
  ...
  if(data["verbose"][0]) {
    ...
    // var msg = map(function(u){
    //   display(u + " : " + n_states.length)
    //   return()
    // }, _.map(globalStore.utterances, 'utt'))
  }
