...
var priorSample = function(par){
  ...
  var p_utts = par.includes("p_utts") ?
...
//TODO: remove this and merge with upper function is the same except for 1st line!
var get_prediction_per_context = function(predictions, context,
                                          posterior_states_context) {
  var posterior_ci = posterior_states_context[context]
  var obj_posterior_ci = Object.values(Object.values(posterior_ci.params)[0])
  var states = _.map(obj_posterior_ci, 'val')
  var states_ids = _.map(states, 'bn_id')
  var p_states = _.map(obj_posterior_ci, 'prob')
  ...
  // display(dims(weights))
  var utts = _.uniqWith(_.map(_.map(distributions_speaker, 'params'), 'vs'), _.isEqual)
  if(utts.length != 1) error("in get_prediction_per_context unequal utterances states")
  var weighted_sums = T.dot(weights, p_utts)
