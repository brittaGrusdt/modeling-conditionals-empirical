...
var priorSample = function(par){
  var gamma = par.includes("gamma") ? {gamma: sample(Uniform({a: 0, b: 1}))} : {};
  // log normal distribution for alpha
  var alpha = par.includes("alpha") ?
    {alpha: Math.exp(sample(Gaussian({mu: 1.5, sigma: 1}),
      {driftKernel: gaussianKernel}))} : {};

  var theta = par.includes("theta") ?
    {theta: sample(Beta({a: 4, b: 2}), {driftKernel: betaKernel})} : {};
