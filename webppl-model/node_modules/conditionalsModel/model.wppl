...
var priorSample = function(par){
  ...
  var cost = par.includes("utt_cost") ?
    Object.fromEntries(map(function(u){
      var cost_u = sample(Gaussian({mu: 0, sigma: 0.25}), {driftKernel: gaussianKernel})
      return([u, cost_u])
    }, _.map(globalStore.utterances, 'utt'))) : {};
  var utt_cost = {'utt_cost': cost}
  //display(cost)
  ...
  var all_pars = Object.assign(gamma, alpha, theta, theta_might, utt_cost, ps_utts); //, utts);
