...
var gaussianKernel = function(prevVal) {
  return Gaussian({mu: prevVal, sigma: .05});
};
...
var priorSample = function(par){
  ...
  var theta = par.includes("theta") ? {theta: sample(Beta({a: 10, b: 2}))} : {};
...
var get_posterior_given_dij = function(P_s_given_ci, slider_rating){
  ...
  var posterior = Infer({model: function(){
    ...
    // factor(-kl)
    var kl = Math.exp(-1 * get_kulback_leibler(observed_table, s.table))
    factor(kl)
    return(s)
  }})
  return(posterior)
