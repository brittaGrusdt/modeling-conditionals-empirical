...
var sample_prior_bg = function(){
  return({
    "shape1": sample(Exponential({a: 0.1}), {driftKernel: exponentialKernel}),
    "shape2": sample(Exponential({a: 0.1}), {driftKernel: exponentialKernel})
  })
}
////////////////////////////////////////////////////////////////////////////////
...
var get_likelihood_bg = function(obs, pars_bg){
  ...
  // probability of observed value to be larger than perfect independnt value
  var ll_sign = Bernoulli({p: pars_bg.theta}).score(sign == 1 ? true : false)
...
var sample_independent_table = function(pars_blue, pars_green, pars_bg){
  ...
  var diff_ratio = sample_zoib(pars_bg)
  var sign = Bernoulli({p: pars_bg.theta}) ? 1 : -1;

  var mu = p_blue * p_green
  var max_add = range_bg.max - mu
  var max_sub = mu - range_bg.min

  var delta = sign < 0 ? sign * diff_ratio * max_sub : diff_ratio * max_add;
  var p_bg = mu + delta;
