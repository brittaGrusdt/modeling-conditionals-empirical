...
var get_P_r_given_context = function(context){
  var distr = context.includes("independent") ?
   ...
   context.includes("if2") ? Categorical({vs: ["A implies C", "C implies A"],
                                          ps: [0.75, 0.25]}) :
   context == "all" ?  Categorical({vs: ["A implies C", "C implies A", "A || C"],
                                    ps: [(0.75+1+0)/3, (0+0+0.25)/3, (1+0+0/3)]}) :
...
// @param distribution string 'zero-one-inflated-beta' or 'gaussian'
    ...
    var likelihood_ci_p = map(function(pars){
      ...
      var distrs = distribution == "zero-one-inflated-beta" ?
        ...
        distribution == "gaussian" ?
          [pars.p, Gaussian({mu: pars.mu, sigma: pars.sigma})] :
          error("only zero-one-inflated-beta and gaussian likelihoods implemented.");
...
/// NEEDS to be revised
// var posterior_given_context = funciton(prior_conditioned_r, context){
//
//   var posterior = Infer({model: function() {
//     var r = sample(get_P_r_given_context(context))
//
//     var s = sample(prior_conditioned_r[r]); // state with relation r
//
//     //likelihood
